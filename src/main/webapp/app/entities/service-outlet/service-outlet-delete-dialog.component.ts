...
@Component({
    selector: 'jhi-service-outlet-delete-dialog',
...
@Component({
    selector: 'jhi-service-outlet-delete-popup',
    template: ''
})
export class ServiceOutletDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ serviceOutlet }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(ServiceOutletDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
